...
run_docker_classic_test() {
  ...
  sleep 2
  go clean -testcache
  go test -count=1 -timeout 30s -run ^${1}$ github.com/acouvreur/sablier/e2e || docker compose -f ${DOCKER_COMPOSE_FILE} -p ${DOCKER_COMPOSE_PROJECT_NAME}"" logs sablier traefik
  destroy_docker_classic
}
